...
- **☁️ النسخ الاحتياطي والمزامنة:** دعم النسخ الاحتياطي محلياً وسحابياً عبر Google Drive، مع التحقق من سلامة المحتوى واستعادة ذرية للبيانات.
...
## منظومة النسخ الاحتياطي

- إصدار حزمة النسخ الحالي: `1.2.0`.
- التحقق من سلامة النسخ الحديثة يغطي جميع البيانات التي تدخل في الاستعادة، بما فيها بيانات العملاء، الملاحظات، سلة المهملات، التصنيفات، الروابط، التثبيتات وترتيب التصنيفات.
- الإصدارات `1.1.0` و`1.1` تستمر في استخدام خوارزمية البصمة السابقة عند الاستعادة لضمان التوافق العكسي.
- الكتابة المحلية تتم عبر ملف مؤقت والتحقق من الحجم والوجود والمسار قبل اعتماد الملف النهائي.
- مزامنة Google Drive تميز بين انتهاء الصلاحية، عدم وجود الملف، الأخطاء العابرة والأخطاء الدائمة، وتمنع تحويل فشل التحديث إلى إنشاء نسخة جديدة إلا في حالة عدم وجود الملف فعلياً.
- ملفات النسخ الحالية ليست مشفرة تشفيراً محمولاً بين الأجهزة؛ الحماية الحالية في هذا المسار هي عزل تخزين التطبيق والتحقق من سلامة المحتوى. لا ينبغي وصف `.mzd` بأنها "مشفرة" ما لم تتم إضافة مفتاح تشفير محمول مع تدفق إدارة مفتاح واضح.
