...
class RecurringScheduleExhaustiveContractTest {
    ...
    private fun config(frequency:String, start:Long, end:Long, last:Long=0, dow:List<Int>=emptyList(), dom:List<Int>=emptyList()) =
        RecurringConfig("r","tx","c","C",BigDecimal("100"),"OWED_BY_THEM","x",frequency,
            dow,dom,10,0,start,end,last,true,false,"ر.ي",BigDecimal.ZERO,BigDecimal.ONE,false,BigDecimal.ZERO)

    @Test fun dailyBoundariesAreInclusiveAndReturnedOnce() {
        ...
        assertEquals(listOf(start/1000,(start+86_400_000)/1000,(start+2*86_400_000)/1000),out)
    }

    @Test fun lastExecutedOccurrenceIsNeverRepeated() {
        val start=millis(2026,1,1); val last=start/1000; val end=millis(2026,1,3)
        val out=RecurringScheduleCalculator.dueOccurrences(config("DAILY",start,end,last),end)
        assertEquals(listOf((start+86_400_000)/1000,(start+2*86_400_000)/1000),out)
    }

    @Test fun maxOccurrencesIsAHardOutputBound() {
        val start=millis(2020,1,1); val end=millis(2026,1,1)
    ...
    @Test fun weeklySelectorUsesOnlyConfiguredWeekdays() {
        val start=millis(2026,1,4); val end=millis(2026,1,11)
    ...
    @Test fun monthlySelectorUsesOnlyConfiguredDays() {
        val start=millis(2026,1,1); val end=millis(2026,2,5)
    ...
    @Test fun unsupportedFrequencyProducesNoOccurrences() {
        val start=millis(2026,1,1); val end=millis(2026,1,10)
