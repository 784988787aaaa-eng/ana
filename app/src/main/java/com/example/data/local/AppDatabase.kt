...
// =====================================================================
// 1) هذا الملف يمثل نقطة تجميع بنية Room؛ أي تغيير مستقبلي في entities أو version
//    يجب أن يقترن بمراجعة صريحة لسجل DatabaseMigrations قبل الدمج.
// 2) يُفضّل مستقبلاً تفعيل exportSchema مع حفظ مخططات Room في مستودع المشروع،
//    إذا كانت سياسة الإصدار تسمح بذلك، لتسهيل التدقيق الآلي في توافق المخطط.
// 3) منطق التنظيف داخل onOpen مفيد كحاجز سلامة، لكنه يستحق اختباراً مستقلاً
//    يغطي القيم الفارغة و"null" والربط الذاتي دون تغيير السلوك الحالي.
// 4) لا ينبغي نقل Singleton أو إضافة مسار إنشاء ثانٍ خارج getDatabase؛ الحفاظ على
//    نقطة الدخول الواحدة يقلل احتمالات وجود أكثر من اتصال Room متزامن.
