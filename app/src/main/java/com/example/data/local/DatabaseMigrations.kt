...
// =====================================================================
// 1) هذا الملف سجل تاريخي غير قابل لإعادة الكتابة؛ الهجرات القديمة جزء من عقد
//    التوافق مع قواعد بيانات المستخدمين الموجودة فعلياً في الأجهزة.
// 2) لا تُدمج هجرات سابقة ولا يُعاد ترقيمها. أي تعديل بنيوي جديد يبدأ من version
//    لاحقة مستقلة ويُضاف إلى ALL_MIGRATIONS في موضعه الزمني الصحيح.
// 3) أوامر CREATE/DROP/RENAME وإعادة بناء الجداول تستحق اختبارات MigrationTest
//    تغطي البيانات قبل وبعد كل انتقال حساس.
// 4) يجب الانتباه إلى تطابق أسماء الأعمدة وأنواعها مع Entities الحالية؛ Room يعتمد
//    على هذا التطابق عند التحقق من المخطط.
// 5) الفهارس والقيود المرجعية جزء من الأداء وسلامة البيانات، وليست تحسينات شكلية.
