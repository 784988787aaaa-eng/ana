...
class BackupEngine(
    ...
    private fun validatePayload(root: JSONObject) {
        ...
        for (i in 0 until root.getJSONArray("recurring").length()) {
            ...
            require(r.has("snapshotVersion")) { "قالب التكرار ناقص: snapshotVersion" }
            require(r.has("rateContext")) { "قالب التكرار ناقص: rateContext" }
