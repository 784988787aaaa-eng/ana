...
/** واجهة الوصول إلى جداول العملاء والحركات المالية الخاصة بدفتر الحبايب. */
@Dao
interface HabayebDao {

    /** يبث جميع العملاء بترتيب ثابت، مع كسر تعادل زمني بالمعرف لمنع تغير ترتيب العرض. */
    @Query("SELECT * FROM habayeb_customers ORDER BY createdAt DESC, id ASC")
    ...
    /** يبث جميع الحركات بترتيب زمني ثابت دون إعادة ترتيب عشوائية عند تساوي التوقيت. */
    @Query("SELECT * FROM habayeb_transactions ORDER BY timestamp DESC, id ASC")
    fun getAllTransactionsFlow(): Flow<List<HabayebTransaction>>

    /** يبث كشف حساب العميل بترتيب ثابت وآمن للعرض المتكرر. */
    @Query("SELECT * FROM habayeb_transactions WHERE customerId = :customerId ORDER BY timestamp DESC, id ASC")
    ...
    @Query("SELECT * FROM habayeb_transactions WHERE customerId = :customerId ORDER BY timestamp DESC, id ASC LIMIT :limit OFFSET :offset")
    suspend fun getTransactionsForCustomerPaged(customerId: String, limit: Int, offset: Int): List<HabayebTransaction>

    @Query("SELECT * FROM habayeb_transactions WHERE customerId = :customerId ORDER BY timestamp DESC, id ASC LIMIT :limit")
