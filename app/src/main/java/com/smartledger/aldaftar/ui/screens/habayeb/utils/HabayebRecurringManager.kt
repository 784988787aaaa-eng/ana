...
data class RecurringConfig(
    ...
    companion object {
        fun fromJsonObject(obj: JSONObject, context: Context? = null): RecurringConfig {
            ...
            val defaultName = context?.getString(com.smartledger.aldaftar.R.string.customer_default_name) ?: "عميل"
                ...
                return try {
                    if (obj.has(key)) {
                        ...
                        when (raw) {
                            ...
                            is String -> if (raw.isNotBlank()) BigDecimal(raw.trim()) else BigDecimal(defaultVal)
