/**
 * =====================================================================
 * ملف: المحلل الرياضي لتقييم المعادلات المالية (.)
 * =====================================================================
 * 
 ...
 * بالاعتماد على [] لتفادي أخطاء الدقة العشرية في العمليات النقدية.
 * 
 * [المسؤوليات المعمارية والتقنية للملف]:
 * 1. المعالجة والتحليل المعجمي للرموز ( ):
 ...
 * 2. دعم إشارة السالب الأحادية (  ):
 *    - السماح بإدخال الأرقام السالبة في بداية المعادلة دون حدوث أخطاء تحليلية.
 * 3. التقييم الحسابي ثنائي المراحل (- ):
 *    - المرحلة الأولى ( 1): تنفيذ عمليات الضرب والقسمة ('*', '/', '×', '÷') مع فحص القسمة على صفر.
 *    - المرحلة الثانية ( 2): تنفيذ عمليات الجمع والطرح ('+', '-').
 * 4. الحساب المالي الدقيق والآمن (   ):
 *    - استخدام التقريب النصف للأعلى [._] بدقة 10 خانات عشرية لمنع الاستثناءات الحسابية.
 */
...
/**
 * [تقييم وحساب التعبير الرياضي البسيط - ]:
 ...
 *
 * @  النص الرياضي المراد حسابه (مثال: "50 + 10 * 2" أو "١٠٠ + ٥٠").
 * @ ناتج العملية الحسابية كـ [] عالي الدقة، أو  إذا كان التعبير غير صالح.
 */
fun evaluateSimpleExpression(expr: String): BigDecimal? {
    ...
    return runCatching {
        ...
        // -----------------------------------------------------------------
        // مرحلة التحليل المعجمي واستخراج الرموز ( )
        // -----------------------------------------------------------------
        ...
        for (i in 0 until operators.size) {
            ...
            if (op == '*' || op == '/') {
                ...
                val res = if (op == '*') {
                    ...
                    prevNum.divide(nextNum, 10, RoundingMode.HALF_EVEN)
                }
                numbers2.add(res)
            } else {
                operators2.add(op)
